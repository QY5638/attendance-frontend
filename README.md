...
- Element Plus
- Axios
- MediaPipe Tasks Vision（浏览器侧活体挑战）
- Vite 5
- Vitest + jsdom
...
- 当前前端默认面向“一台带摄像头的电脑”运行。
- 人脸录入、打卡和活体挑战均使用浏览器访问本机摄像头，不需要额外考勤机、红外相机、深度摄像头或其他外设。
- 浏览器端活体挑战依赖仓库内已本地化的 MediaPipe 资源：`public/mediapipe/wasm/` 和 `public/models/face_landmarker.task`。
- 建议使用最新版 Chrome 或 Edge 进行联调，以保证摄像头权限、WebAssembly 和视频推理兼容性。
...
表现：活体挑战无法开始、浏览器提示权限拒绝、页面显示“无法访问摄像头”。

排查方式：

- 确认浏览器已允许当前站点访问摄像头
- 确认当前电脑存在可用摄像头，且未被会议软件、录屏软件等其他应用独占
- 优先使用 Chrome 或 Edge 的最新稳定版

### 5. 活体挑战误判较多

表现：页面反复提示脸部未居中、光线不足、头部倾斜，或者动作难以完成。

排查方式：

- 保持脸部位于画面中央，避免过近、过远或明显歪头
- 避免逆光、强灯直射和极暗环境
- 确认镜头内仅有一个人
- 如果是本地图片上传模式，当前环境启用了活体时应切换到摄像头模式完成挑战
